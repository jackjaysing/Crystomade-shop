...
ALTER TABLE coupons ADD COLUMN IF NOT EXISTS redeem_mode TEXT NOT NULL DEFAULT 'checkout';
ALTER TABLE coupons ADD COLUMN IF NOT EXISTS source_raffle_id UUID REFERENCES raffles(id) ON DELETE SET NULL;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'coupons_redeem_mode_check'
  ) THEN
    ALTER TABLE coupons ADD CONSTRAINT coupons_redeem_mode_check
      CHECK (redeem_mode IN ('checkout', 'cart'));
  END IF;
END $$;
...
-- 會員優惠券：支援「已放入購物車」
-- ------------------------------------------------------------
ALTER TABLE member_coupons DROP CONSTRAINT IF EXISTS member_coupons_status_check;
ALTER TABLE member_coupons ADD CONSTRAINT member_coupons_status_check
  CHECK (status IN ('available', 'used', 'expired', 'in_cart'));

-- ------------------------------------------------------------
-- 抽獎活動：獎品禮物券
-- ------------------------------------------------------------
ALTER TABLE raffles ADD COLUMN IF NOT EXISTS prize_title TEXT;
ALTER TABLE raffles ADD COLUMN IF NOT EXISTS prize_image_url TEXT;
ALTER TABLE raffles ADD COLUMN IF NOT EXISTS prize_gift_description TEXT;
ALTER TABLE raffles ADD COLUMN IF NOT EXISTS prize_coupon_id UUID REFERENCES coupons(id) ON DELETE SET NULL;
...
BEGIN
  ...
  IF v_raffle.prize_coupon_id IS NOT NULL THEN
    PERFORM admin_issue_coupon_to_member(v_raffle.prize_coupon_id, v_winner);
  END IF;
...
-- ------------------------------------------------------------
-- 兌換禮物券至購物車（標記 in_cart）
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION redeem_gift_coupon_to_cart(
...
BEGIN
  ...
  IF v_mc.user_id <> p_user_id THEN
    RAISE EXCEPTION '無權限使用此優惠券';
  END IF;

  IF v_mc.status <> 'available' THEN
    RAISE EXCEPTION '此優惠券無法兌換';
  END IF;
  ...
  IF v_coupon.coupon_type <> 'gift' OR v_coupon.redeem_mode <> 'cart' THEN
    RAISE EXCEPTION '此優惠券不支援兌換至購物車';
  END IF;

  UPDATE member_coupons SET status = 'in_cart' WHERE id = p_member_coupon_id;

  RETURN jsonb_build_object(
    'member_coupon_id', v_mc.id,
    'title', v_coupon.title,
    'gift_description', COALESCE(v_coupon.gift_description, v_coupon.title),
...
-- 從購物車移回可用（取消兌換預留）
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION release_gift_coupon_from_cart(
  p_member_coupon_id UUID,
  p_user_id UUID
)
RETURNS VOID
...
BEGIN
  ...
  IF v_mc.user_id <> p_user_id THEN
    RAISE EXCEPTION '無權限';
  END IF;
  IF v_mc.status = 'in_cart' THEN
    UPDATE member_coupons SET status = 'available' WHERE id = p_member_coupon_id;
  END IF;
END;
$$;
...
GRANT EXECUTE ON FUNCTION redeem_gift_coupon_to_cart(UUID, UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION release_gift_coupon_from_cart(UUID, UUID) TO anon, authenticated;
