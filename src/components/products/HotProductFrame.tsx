...
export function HotProductFrame({
  ...
  const innerClass = topOnly
    ? `relative overflow-hidden bg-graphite ${innerRadius}`
    : `relative h-full overflow-hidden ${innerRadius}`

  return (
    <div
      className={`relative flex h-full flex-col bg-gradient-to-br from-amber-glow/90 via-orange-400/55 to-amber-deep/75 p-[2px] shadow-[0_0_28px_rgba(212,165,116,0.22)] ${radius} ${className}`}
    >
      <div
